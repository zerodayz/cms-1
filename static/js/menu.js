...
class MenubarNavigation {
    ...
    toggleStatus(switchNode) {
        const currentState =
            switchNode.getAttribute('aria-checked') === 'true';
        const newState = String(!currentState);

        switchNode.setAttribute('aria-checked', newState);
        // replace css root variables to dark mode
        if (newState === 'true') {
    ...
    initMenu(menu, depth) {
        ...
        for (var i = 0; i < menuitems.length; i++) {
            ...
            if (role.indexOf('menuitem') < 0 && role.indexOf('switch') < 0) {
                continue;
            }
            menuitem.tabIndex = -1;
            ...
            if (role.indexOf('switch') >= 0) {
                menuitem.addEventListener('click', () => this.toggleStatus(menuitem));
                menuitem.addEventListener('keydown', this.onKeydownSwitch.bind(this));
    ...
    openPopup(menuId, menuitem) {
        ...
        let isRightMenu = menuitem.classList.contains('right-menuitem');
        if (popupMenu) {
            ...
            if (isRightMenu) {
                ...
                if (this.isPopup[menuId]) {
                    ...
                    popupMenu.style.right = rect.width + 10 + 'px';
...
window.addEventListener('load', function () {
    ...
    // var firstGridCell = document.querySelector('#home');
    //
