...
class NavigationSpaceContentGenerator {
    ...
    renderParagraph(linkURL, linkName) {
        var content = '';
        if (linkURL === undefined || linkName === undefined) {
            return content;
        }
        let url = linkURL.split('#')[1];
        if (url.length === 0) {
            return content;
        }
...
class TreeViewNavigation {
    constructor(node) {
        ...
        this.contentGenerator = new NavigationSpaceContentGenerator();
    ...
    updateContent(linkURL, linkName, moveFocus) {
        ...
        h2Node = document.querySelector('.post-content h2');
        if (h2Node) {
    ...
    onLinkClick(event) {
        ...
        var content = tgt.textContent.trim().split('\n')[0];
        this.updateContent(tgt.href, content);
